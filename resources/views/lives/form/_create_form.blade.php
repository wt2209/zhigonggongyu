<form action="{{route('lives.store')}}" method="post" accept-charset="UTF-8" class="form-horizontal" pjax-container="">
        ...
        <div class="fields-group">
                    ...
                    <div class="input-group">
                        ...
                        >
                            @foreach($types as $id=>$title)
                                <option value="{{$id}}" @if($room->type_id === $id) selected="selected" @endif>{{$title}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group {!! !$errors->has('person.gender') ? '' : 'has-error' !!}  ">
                ...
                <div class="col-sm-8">
                    ...
                    @else
                        <label class="radio-inline">
                            <input type="radio" name="person[gender]" class="gender" value="男" checked > 男
                        </label>
                        <label class="radio-inline">
                            <input type="radio" name="person[gender]" class="gender" value="女"> 女
                        </label>
                    @endif
                </div>
            </div>
            ...
            <div class="form-group {!! !$errors->has('person.education') ? '' : 'has-error' !!}  ">
                ...
                <div class="col-sm-8">
                    ...
                    @else
                        @foreach(\App\Models\Person::$educationMap as $key => $edu)
                            <label class="radio-inline">
                                @if($key == \App\Models\Person::EDUCATION_UNKNOWN)
                                    <input type="radio" name="person[education]" class="education" value="{{$key}}" checked> {{$edu}}
                                @else
                                    <input type="radio" name="person[education]" class="education" value="{{$key}}"> {{$edu}}
                                @endif
            ...
            <div class="form-group {!! !$errors->has('person.contract_start') ? '' : 'has-error' !!}  ">
                ...
                <div class="col-sm-8">
                    ...
                    <div class="input-group">
                        <input style="width: 200px" type="text"
                               value="{{old('person.contract_start')}}"
                    ...
                    <div class="form-inline">
                        <div class="input-group">
                            <input style="width: 200px" type="text"
                                   ...
                                   value="{{old('person.contract_end') === '无固定期' ? \App\Models\Person::CONTRACT_RETIRE_END : old('person.contract_end')}}"
                                   name="person[contract_end]" class="form-control" placeholder="格式：2018-9-4">
                        </div>
                        <div class="checkbox" style="margin-left: 10px;">
                            <label>
                                <input type="checkbox" id="no-deadline"
                                    @if (old('person.contract_end') === '无固定期')
                                        checked
                                   @endif
                                > 无固定期
                            </label>
            ...
            <div class="form-group {!! !$errors->has('person.remark') ? '' : 'has-error' !!}  ">
                ...
                <div class="col-sm-8">
                    @if($errors->has('person.remark'))
                        ...
                        @endforeach
                    @endif
                    <textarea name="person[remark]" class="form-control" rows="5" placeholder="输入 备注">{{old('person.remark')}}</textarea>
                </div>
            </div>
        </div>
    </div>
